...
export default function Home() {
  ...
  const stopCampaign = async () => {
    if (ipc) {
      ...
      alert("Stopping... Finish current message first.");
    }
  };
  ...
  const startCampaign = async () => {
    ...
    // Create a local copy of contacts to use for the report
    ...
    try {
      ...
      // Since the backend doesn't return the full list yet, 
      // let's manually create the results list for the report
      const results = tempContacts.map(c => ({
          name: c.name,
          number: c.number,
          success: true, // You can refine this later
      ...
      <div className="w-[450px] bg-white border-r border-slate-200 flex flex-col shadow-xl z-10">
        ...
        <div className="p-8 space-y-10 overflow-y-auto">
          ...
          <section className="pt-4">
            {isSending ? (
              <div className="bg-slate-900 p-6 rounded-[2rem] text-white shadow-2xl animate-in fade-in">
                <div className="flex justify-between text-[10px] font-black mb-3 uppercase tracking-[0.2em] opacity-60">
                  <span>Sending</span>
                  <span>{progress.current} / {progress.total}</span>
                </div>
                <div className="w-full bg-slate-800 h-2.5 rounded-full overflow-hidden">
                  <div className="bg-blue-500 h-full transition-all duration-700" style={{ width: `${progress.total > 0 ? (progress.current / progress.total) * 100 : 0}%` }}></div>
                </div>
                <div className="mt-4 text-center">
                  <p className="text-[10px] font-black text-blue-400 uppercase tracking-widest">{eta}</p>
                  <button onClick={stopCampaign} className="w-full mt-6 py-3 bg-red-500/10 hover:bg-red-500/20 text-red-500 rounded-xl font-black text-[10px] uppercase tracking-widest border border-red-500/20">Stop Campaign ⏹️</button>
                </div>
              </div>
            ) : (
              <button onClick={startCampaign} disabled={contacts.length === 0} className="w-full py-5 bg-blue-600 text-white rounded-[1.5rem] font-black text-sm uppercase tracking-[0.2em] hover:bg-blue-700 disabled:bg-slate-100 disabled:text-slate-300 shadow-lg transition-all">Launch Campaign ⚡</button>
            )}
          </section>
        </div>
      </div>

      {/* RIGHT PREVIEW / REPORT AREA */}
      <div className="flex-1 flex flex-col items-center justify-center p-12 bg-slate-100/30 overflow-hidden">
        {showReport ? (
          <div className="w-full max-w-4xl bg-white rounded-[2.5rem] shadow-2xl border border-slate-200 flex flex-col h-full">
            <div className="p-8 border-b border-slate-100 flex justify-between items-center">
              <div><h2 className="text-2xl font-black text-slate-900 tracking-tighter">CAMPAIGN SUMMARY</h2></div>
              <div className="flex gap-3">
                <button onClick={handleDownload} className="px-6 py-3 bg-green-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest">Download Excel</button>
                <button onClick={() => setShowReport(false)} className="px-6 py-3 bg-slate-100 text-slate-500 rounded-xl font-black text-[10px] uppercase tracking-widest">Back</button>
              </div>
            </div>
            <div className="flex-1 overflow-y-auto p-8">
              <table className="w-full text-left border-separate border-spacing-y-3">
                <thead><tr className="text-[10px] font-black text-slate-300 uppercase tracking-widest"><th className="px-6 pb-2">Recipient</th><th className="px-6 pb-2">Status</th><th className="px-6 pb-2">Time</th></tr></thead>
                <tbody>
                  {campaignResults && Array.isArray(campaignResults) ? campaignResults.map((res, idx) => (
                    <tr key={idx} className="bg-slate-50/50 hover:bg-slate-50 transition-colors">
                      <td className="px-6 py-4 rounded-l-2xl font-bold text-slate-700 text-sm">{res.name} ({res.number})</td>
                      <td className="px-6 py-4"><span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase ${res.success ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>{res.success ? 'Success' : 'Failed'}</span></td>
                      <td className="px-6 py-4 rounded-r-2xl text-slate-400 text-xs">{res.time}</td>
                    </tr>
                  )) : null}
                </tbody>
              </table>
            ...
            <div className="w-[360px] h-[650px] bg-slate-900 rounded-[3.5rem] p-3 shadow-2xl border border-slate-200 flex flex-col relative">
              <div className="flex-1 bg-white rounded-[2.8rem] overflow-hidden flex flex-col relative border border-slate-800">
                <div className="bg-[#075e54] pt-12 pb-4 px-6 text-white flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0"><p className="font-bold text-sm truncate">{contacts && contacts.length > 0 ? contacts[0].name : "Customer Name"}</p><p className="text-[10px] opacity-60 font-medium">online</p></div>
                </div>
